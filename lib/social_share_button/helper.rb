...
module SocialShareButton
  module Helper
    def social_share_button_tag(title = "", opts = {})
      ...
      rel = opts[:rel]
      html = []
      html << "<div class='social-share-button' data-title='#{h title}' data-img='#{opts[:image]}' data-url='#{opts[:url]}' data-desc='#{opts[:desc]}' data-via='#{opts[:via]}' data-referral='#{opts[:referral]}'>"
